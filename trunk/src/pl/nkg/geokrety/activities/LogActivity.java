...
public class LogActivity extends ManagedDialogsActivity implements LocationListener {
	...
	public void dialogFinished(final AbstractDialogWrapper<?> dialog, final int buttonId, final Serializable arg) {
		switch (dialog.getDialogId()) {
			...
			case Dialogs.DATE_PICKERDIALOG:
				storeToGeoKretLog(currentLog);
				currentLog.setDate(datePickerDialog.getYear(), datePickerDialog.getMonthOfYear() + 1, datePickerDialog.getDayOfMonth());
				loadFromGeoKretLog(currentLog);
				break;

			case Dialogs.INVENTORY_SPINNERDIALOG:
				final Geokret kret = (Geokret) inventorySpinnerDialog.getAdapter().getItem(buttonId);
				trackingCodeEditText.setText(kret.getTackingCode());
				break;

			case Dialogs.OCS_SPINNERDIALOG:
				final GeocacheLog log = (GeocacheLog) ocsSpinnerDialog.getAdapter().getItem(buttonId);
	...
	public void onLocationChanged(final Location location) {
		coordinatesEditText.setText(//
				Utils.latlonFormat.format(location.getLatitude()) + ' ' + Utils.latlonFormat.format(location.getLongitude()));
	...
	private void configAdapters() {
		...
		ocsSpinnerDialog.setAdapter(new ArrayAdapter<GeocacheLog>(this, android.R.layout.simple_list_item_single_choice, currentAccount.getOpenCachingLogs()));
	}
	...
	private void updateCurrentAccount(final boolean always, final boolean force) {
		final StateHolder holder = ((GeoKretyApplication) getApplication()).getStateHolder();
		final int currentAccountNr = holder.getDefaultAccount();
		if (currentAccountNr != AdapterView.INVALID_POSITION) {
